...
quick_main!(|| -> Result<()> {
    ...
    if args.input_option.is_some() {
        ...
        input_reader = Box::new(BufReader::new(input_file));
    } else {
        input_reader = Box::new(BufReader::new(stdin_handle.lock()));
    }

    // Setup a buffered output reader
    // Use output file if providd, otherwise use stdout
    let mut output_writer: Box<Write>;
    if args.output_option.is_some() {
        ...
        output_writer = Box::new(BufWriter::new(output_file));
    } else {
        output_writer = Box::new(stdout_handle.lock());
    }
    ...
    input_reader.read_to_string(&mut input_buffer).chain_err(
        || {
            format!("Couldn't read input contents")
        },
    )?;
    ...
    let output;
    if args.not_pretty {
        output = json::stringify(json_object);
    } else {
        output = json::stringify_pretty(json_object, args.indent_spaces);
    }
